...
// FindAndCompress updates the label of the starting pixel at each iteration of
// the loop. This means that, if the equivalence tree is like this:
...
// then the first iteration updates the label of C, assigning it value B, and
// the second iteration assigns A. This way, another thread reading C during the
// process will find an updated value and will avoid a step. FindCompresso
// performs better than Find only sometimes (rarely?).
...
//__device__ __forceinline__ void SetBit(unsigned char &bitmap, unsigned char
//pos) { 	bitmap |= (1 << pos);
//}
...
REGISTER_KERNELS(BUF, InitLabeling, Compression, Merge, FinalLabeling)
