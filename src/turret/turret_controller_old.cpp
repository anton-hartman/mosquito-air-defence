#include "turret_controller_old.hpp"
#include <cstdlib>
...
void auto_control(std::pair<int, int> actual_pos,
  ...
  x_target_angle = target_pos.first;
  m2_target_angle = target_pos.second;

  if (m1_actual_angle != x_target_angle) {
    driver::select_motor(MOTOR1);
    steps = abs(x_target_angle - m1_actual_angle) / MICROSTEP_ANGLE;
    if (m1_actual_angle < x_target_angle) {
    ...
    m1_actual_angle = x_target_angle;
  }
