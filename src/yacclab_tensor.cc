...
void YacclabTensorOutput2D::WriteColored(const std::string& filename) const {
  ...
  for (int r = 0; r < mat_.rows; ++r) {
    ...
    for (int c = 0; c < mat_.cols; ++c) {
      mat_colored_row[c] = Vec3b(mat_row[c] * 131 % 255, mat_row[c] * 241 % 255,
                                 mat_row[c] * 251 % 255);
    }
  }
  ...
  if (mat_.dims == 3) {
    for (int z = 0; z < mat_.size[0]; z++) {
      for (int y = 0; y < mat_.size[1]; y++) {
        unsigned int* img_labels_row = reinterpret_cast<unsigned int*>(
            mat_.data + z * mat_.step[0] + y * mat_.step[1]);
...
void YacclabTensorOutput3D::WriteColored(const std::string& filename) const {
  ...
  for (int z = 0; z < mat_.size[0]; z++) {
    for (int y = 0; y < mat_.size[1]; y++) {
      ...
      for (int x = 0; x < mat_.size[2]; x++) {
        img_out_row[x] =
            Vec3b(img_labels_row[x] * 131 % 255, img_labels_row[x] * 241 % 255,
                  img_labels_row[x] * 251 % 255);
      }
    }
  }
