...
//	std::string s = "\"{/:Bold CPU}: " + s_info.cpu() + " {/:Bold BUILD}: "
//+ s_info.build() + " {/:Bold OS}: " + s_info.os() + 		" {/:Bold COMPILER}: " +
//s_info.compiler_name() + " " + s_info.compiler_version() + "\" font ', 9'";
//	return s;
// }
// template <Connectivity Conn>
// std::string Labeling3D<Conn>::GetTitle(const SystemInfo& s_info) {
//     std::string s = "\"{/:Bold CPU}: " + s_info.cpu() + " {/:Bold BUILD}: " +
//     s_info.build() + " {/:Bold OS}: " + s_info.os() +
//         " {/:Bold COMPILER}: " + s_info.compiler_name() + " " +
...
//     CudaInfo cuda_info;
//     std::string s = "\"{/:Bold GPU}: " + cuda_info.device_name_ + " {/:Bold
//     CUDA Capability}: " + cuda_info.cuda_capability_ +
//         " {/:Bold Runtime}: " + cuda_info.runtime_version_ + " {/:Bold
//         Driver}: " + cuda_info.driver_version_;
//     return s;
// }
// template <Connectivity Conn>
...
//     const Labeling2D<Conn> *correct_alg_2 = dynamic_cast<const
//     Labeling2D<Conn>*>(correct_alg); if (correct_alg_2 == nullptr)
//         return false;
...
//     // correct_alg already ran
//     const Labeling3D<Conn> *correct_alg_3 = dynamic_cast<const
//     Labeling3D<Conn>*>(correct_alg); if (correct_alg_3 == nullptr)
//         return false;
...
bool LabelingMapSingleton::Exists(const std::string& s) {
  return LabelingMapSingleton::GetInstance().data_.end() !=
         LabelingMapSingleton::GetInstance().data_.find(s);
}
...
const std::vector<std::string>& KernelMapSingleton::GetKernels(
    const std::string& s) {
  return KernelMapSingleton::GetInstance().data_.at(s);
}

bool KernelMapSingleton::Exists(const std::string& s) {
  return KernelMapSingleton::GetInstance().data_.end() !=
         KernelMapSingleton::GetInstance().data_.find(s);
}

void KernelMapSingleton::InitializeAlgorithm(const std::string& algorithm,
