...
__global__ void InitLabeling(const cuda::PtrStepSzb img,
                             cuda::PtrStepSzi labels,
    ...
    *(reinterpret_cast<int*>(buffer)) = 0;
    ...
    if (col + 1 < img.cols) {
      ...
      *(reinterpret_cast<int16_t*>(buffer)) =
          *(reinterpret_cast<int16_t*>(img.data + img_index));

      if (row + 1 < img.rows) {
        *(reinterpret_cast<int16_t*>(buffer + 2)) =
            *(reinterpret_cast<int16_t*>(img.data + img_index + img.step));
      }
    } else {
    ...
    // Q square
    if ((HasBit(P, 1) && img.data[img_index - img.step]) ||
        (HasBit(P, 2) && img.data[img_index + 1 - img.step])) {
      if (!father_offset) {
    ...
    // S square
    if ((HasBit(P, 4) && img.data[img_index - 1]) ||
        (HasBit(P, 8) && img.data[img_index + img.step - 1])) {
      if (!father_offset) {
        father_offset = -2;
      } else {
        SetBit(info, Info::S);
    ...
    if (col + 1 < labels.cols) {
      last_pixel =
          reinterpret_cast<unsigned char*>(labels.data + labels_index + 1);
    } else if (row + 1 < labels.rows) {
      last_pixel = reinterpret_cast<unsigned char*>(
          labels.data + labels_index + labels.step / labels.elem_size);
    }
    *last_pixel = info;
  }
}
...
__global__ void Merge(cuda::PtrStepSzi labels, unsigned char* last_pixel) {
  ...
  if (row < labels.rows && col < labels.cols) {
    ...
    if (HasBit(info, Info::Q)) {
      Union(labels.data, labels_index,
            labels_index - 2 * (labels.step / labels.elem_size));
    }
    if (HasBit(info, Info::R)) {
      Union(labels.data, labels_index,
            labels_index - 2 * (labels.step / labels.elem_size) + 2);
    }
...
__global__ void FinalLabeling(const cuda::PtrStepSzb img,
                              cuda::PtrStepSzi labels) {
  unsigned row = (blockIdx.y * blockDim.y + threadIdx.y) * 2;
  unsigned col = (blockIdx.x * blockDim.x + threadIdx.x) * 2;
  unsigned labels_index = row * (labels.step / labels.elem_size) + col;
  ...
  if (row < labels.rows && col < labels.cols) {
    ...
    if (col + 1 < labels.cols) {
      *reinterpret_cast<unsigned long long*>(labels.data + labels_index) =
          (static_cast<unsigned long long>(HasBit(info, Info::b) * label)
           << 32) |
          (HasBit(info, Info::a) * label);

      if (row + 1 < labels.rows) {
        *reinterpret_cast<unsigned long long*>(labels.data + labels_index +
                                               labels.step / labels.elem_size) =
            (static_cast<unsigned long long>(HasBit(info, Info::d) * label)
             << 32) |
            (HasBit(info, Info::c) * label);
      }
    } else {
      labels[labels_index] = HasBit(info, Info::a) * label;

      if (row + 1 < labels.rows) {
        labels[labels_index + (labels.step / labels.elem_size)] =
            HasBit(info, Info::c) * label;
      }
    }
  }
...
class BKE : public GpuLabeling2D<Connectivity2D::CONN_8> {
 ...
 public:
  ...
  void PerformLabeling() {
    ...
    if ((d_img_.rows == 1 || d_img_.cols == 1) &&
        !((d_img_.rows + d_img_.cols) % 2)) {
      cudaMalloc(&last_pixel_, sizeof(unsigned char));
      last_pixel_allocated_ = true;
    } else {
      last_pixel_ = d_img_labels_.data +
                    ((d_img_labels_.rows - 2) * d_img_labels_.step) +
                    (d_img_labels_.cols - 2) * d_img_labels_.elemSize();
    }

    grid_size_ =
        dim3((((d_img_.cols + 1) / 2) + BLOCK_COLS - 1) / BLOCK_COLS,
             (((d_img_.rows + 1) / 2) + BLOCK_ROWS - 1) / BLOCK_ROWS, 1);
    ...
    // d_expanded_connections.create(d_connections_.rows * 3,
    // d_connections_.cols * 3, CV_8UC1); ExpandConnections << <grid_size_,
    // block_size_ >> > (d_connections_, d_expanded_connections); Mat1b
  ...
  void PerformLabelingBlocksize(int x, int y, int z) override {
    ...
    grid_size_ = dim3((((d_img_.cols + 1) / 2) + x - 1) / x,
                      (((d_img_.rows + 1) / 2) + y - 1) / y, 1);
    block_size_ = dim3(x, y, 1);

    BLOCKSIZE_KERNEL(InitLabeling, grid_size_, block_size_, 0, d_img_,
                     d_img_labels_, last_pixel_)
    ...
    BLOCKSIZE_KERNEL(FinalLabeling, grid_size_, block_size_, 0, d_img_,
                     d_img_labels_)
 ...
 private:
  double Alloc() {
    ...
    cudaMemset2D(d_img_labels_.data, d_img_labels_.step, 0,
                 d_img_labels_.cols * 4, d_img_labels_.rows);
    if (last_pixel_allocated_) {
  ...
  void MemoryTransferDeviceToHost() { d_img_labels_.download(img_labels_); }
