...
#define UPPER_BOUND_4_CONNECTIVITY \
  (((size_t)img_.rows * (size_t)img_.cols + 1) / 2 + 1)
#define UPPER_BOUND_8_CONNECTIVITY \
  ((size_t)((img_.rows + 1) / 2) * (size_t)((img_.cols + 1) / 2) + 1)

#define UPPER_BOUND_6_CONNECTIVITY                                            \
  (((size_t)img_.size[0] * (size_t)img_.size[1] * (size_t)img_.size[2] + 1) / \
       2 +                                                                    \
   1)
#define UPPER_BOUND_26_CONNECTIVITY                                      \
  ((size_t)((img_.size[0] + 1) / 2) * (size_t)((img_.size[1] + 1) / 2) * \
       (size_t)((img_.size[2] + 1) / 2) +                                \
   1)
...
class Labeling {
 public:
  ...
  Labeling(std::unique_ptr<YacclabTensorInput> input,
           std::unique_ptr<YacclabTensorOutput> output)
      : input_(std::move(input)), output_(std::move(output)) {}

  virtual ~Labeling() = default;

  virtual void PerformLabeling() {
    throw std::runtime_error("'PerformLabeling()' not implemented");
  }
  virtual void PerformLabelingWithSteps() {
    throw std::runtime_error("'PerformLabelingWithSteps()' not implemented");
  }
  virtual void PerformLabelingMem(std::vector<uint64_t>& accesses) {
    throw std::runtime_error("'PerformLabelingMem(...)' not implemented");
  }
...
class Labeling2D : public Labeling {
 public:
  ...
  Labeling2D(std::unique_ptr<YacclabTensorInput2D> input,
             std::unique_ptr<YacclabTensorOutput2D> output)
      : Labeling(std::move(input), std::move(output)),
        img_((dynamic_cast<YacclabTensorInput2D*>(input_.get()))->Raw()),
        img_labels_(
            (dynamic_cast<YacclabTensorOutput2D*>(output_.get()))->Raw()) {}

  Labeling2D()
      : Labeling2D(std::make_unique<YacclabTensorInput2D>(),
                   std::make_unique<YacclabTensorOutput2D>()) {}

  virtual ~Labeling2D() = default;

  virtual std::string CheckAlg() const {
    return LabelingCheckSingleton2D::GetCheckAlg(Conn, LabelBackground);
  }
...
class Labeling3D : public Labeling {
 public:
  ...
  Labeling3D(std::unique_ptr<YacclabTensorInput3D> input,
             std::unique_ptr<YacclabTensorOutput3D> output)
      : Labeling(std::move(input), std::move(output)),
        img_((dynamic_cast<YacclabTensorInput3D*>(input_.get()))->Raw()),
        img_labels_(
            (dynamic_cast<YacclabTensorOutput3D*>(output_.get()))->Raw()) {}

  Labeling3D()
      : Labeling3D(std::make_unique<YacclabTensorInput3D>(),
                   std::make_unique<YacclabTensorOutput3D>()) {}

  virtual ~Labeling3D() = default;

  virtual std::string CheckAlg() const {
    return LabelingCheckSingleton3D::GetCheckAlg(Conn, LabelBackground);
  }
...
class GpuLabeling2D : public Labeling2D<Conn, LabelBackground> {
 public:
  ...
  GpuLabeling2D()
      : Labeling2D<Conn>(std::make_unique<YacclabTensorInput2DCuda>(),
                         std::make_unique<YacclabTensorOutput2DCuda>()),
        d_img_(dynamic_cast<YacclabTensorInput2DCuda*>(input_.get())->GpuRaw()),
        d_img_labels_(
            dynamic_cast<YacclabTensorOutput2DCuda*>(output_.get())->GpuRaw()) {
  }
...
class GpuLabeling3D : public Labeling3D<Conn, LabelBackground> {
 public:
  ...
  GpuLabeling3D()
      : Labeling3D<Conn>(std::make_unique<YacclabTensorInput3DCuda>(),
                         std::make_unique<YacclabTensorOutput3DCuda>()),
        d_img_(
            (dynamic_cast<YacclabTensorInput3DCuda*>(input_.get()))->GpuRaw()),
        d_img_labels_((dynamic_cast<YacclabTensorOutput3DCuda*>(output_.get()))
                          ->GpuRaw()) {}
...
class LabelingMapSingleton {
 ...
 private:
  ...
  ~LabelingMapSingleton() {
    for (std::map<std::string, Labeling*>::iterator it = data_.begin();
         it != data_.end(); ++it)
      delete it->second;
  }
};
...
class KernelMapSingleton {
 public:
  ...
  static bool Exists(const std::string& s);
  static void InitializeAlgorithm(const std::string& algorithm,
